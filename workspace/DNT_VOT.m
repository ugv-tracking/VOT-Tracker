...
chnum = 512;
addpath(genpath('/home/xulingyun/Documents/MATALB/VOT2016/vot-toolkit/workspace/DNT/'));

rand('state', 0);
[handle, im1_name, region] = vot('rectangle');
im1 = double(imread(im1_name));
init_rec = region2location(im1, region);
trackparam;
% *************************************************************
...
while true
    ...
    % VOT: Get next frame
    % **********************************
    [handle, img_name] = handle.frame(handle);
    caffe('set_phase_test');
    location_last = floor(location);
    tic;
    fprintf('Processing image: %d/%d\n', img_id, fnum);
    
    if isempty(img_name) % Are we done?
        break;
    end;
...
%     set(gca,'xtick',[],'ytick',[]);
%     axis off;
%     imwrite(frame2im(getframe(gcf)), sprintf('/home/zhizhen/Desktop/david3/%s.jpg', 'high_image_sum'));
% 
...
%     axis off;
%     imwrite(frame2im(getframe(gcf)), sprintf('/home/zhizhen/Desktop/david3/%s.jpg', 'low_image_sum'));
% catch
% end
